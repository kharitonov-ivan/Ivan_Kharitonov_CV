% DOCUMENT SETUP
\documentclass[12pt, a4paper]{extarticle}
\makeatletter
\renewcommand\normalsize{\@setfontsize\normalsize{9}{12}} % FONT SIZE
\makeatother
\linespread{1.25} % LINE SPREAD
\usepackage{array, xcolor, lipsum, bibentry}
\usepackage[left=1cm, right=1cm,top=1.5cm, bottom=1cm]{geometry} % PAGE MARGINS
\usepackage{multicol}
\usepackage{titling}
\usepackage{tikz}
\usepackage{enumitem}
\pagenumbering{gobble}  % DISABLE PAGE NUMBERING
%  DISABLE LINE BREAKING
\hyphenpenalty = 10000 
\exhyphenpenalty = 10000
% 
\definecolor{mygray}{gray}{0.35}
\newcommand\boldgrey[1]{\textcolor{mygray}{\textbf{#1}}} 
\newcommand{\size}[2]{{\fontsize{#1}{0}\selectfont#2}}
\usetikzlibrary{arrows}
\usetikzlibrary{shapes}
\definecolor{lightblue}{RGB}{ 181 212 239}
\usepackage{tcolorbox}
\newtcbox{\mymk}[1][lightblue]{on line, arc=7pt,colback=#1!10!white,colframe=#1!50!black,
  before upper={\rule[-3pt]{0pt}{10pt}},boxrule=1pt, boxsep=0pt,left=6pt,
  right=6pt,top=2pt,bottom=2pt}
% \newcommand{\mymk}[1]{%
%   \tikz[baseline=0pt]\node[draw,rectangle,fill=lightblue, rounded corners, minimum height=1.6em,inner sep=3pt, outer sep=3pt](char){#1} ;}
% % 
\newcommand*\circled[1]{\tikz[baseline=(char.base)]{
            \node[shape=circle,draw,inner sep=2pt] (char) {#1};}}

% NEW BULLET POINTS
\renewcommand{\labelitemi}{$\circ$} 
\renewcommand\labelitemii{$\square$}
% HYPERLINKS SETUP
\usepackage{hyperref}
\hypersetup{
colorlinks=true, 
urlcolor=blue,  
urlbordercolor=cyan}
% DEFINE SPACE AROUND SECTION
\usepackage{titlesec}
% \titlespacing*{<command>}{<left>}{<before-sep>}{<after-sep>}
\titlespacing*{\section} {0pt}{0pt}{0pt} 
% PHOTO
\usepackage{graphicx}
\usepackage{caption}
\graphicspath{{./images/}}
\usepackage[leftcaption]{sidecap}
\usepackage{enumitem}

\newcommand{\placeheadding}[3]{\textbf{#1} \textit{#2} \hfill #3\null}
\newcommand{\positionheading}[2]{\par\noindent \textsc{#1} \hfill #2}
\newcommand{\ressubheading}[5]{\placeheadding{#1}{#2}{#3} \positionheading{#4}{#5}}
% \newcommand{\ressubheading}[5]{\textbf{#1} \textit{#2} \hfill #3\null \\ \textsc{#4} \hfill #5}

\setlength{\parskip}{0cm}
\setlength{\parindent}{1em}
\newcommand{\horizontalline}{\par\noindent\\}
%
\newcommand*{\sectionformat}{\centering}
%
%--------------------TABLE SETUP----------------------
\definecolor{lightgray}{gray}{0.65}
\newcolumntype{L}{>{\raggedleft}p{0.2\textwidth}} % LEFT TABLE MARGIN
\newcolumntype{R}{p{0.8\textwidth}} % RIGHT TABLE MARGIN
\newcommand\VRule{\color{lightgray}\vrule width 0.5pt}
% 
\begin{document}
\input{parts/contacts.tex}
\input{parts/about.tex}
\input{parts/education.tex}
\input{parts/experience.tex}
\input{parts/projects.tex}
\input{parts/teaching.tex}
\input{parts/skills.tex}
\input{parts/activities.tex}
\end{document}